...
def PredictTheWinner(nums: List[int]) -> bool:
    def rec_helper(start_idx, end_idx, turn):
        if start_idx == end_idx:
            return turn * nums[start_idx]

        first_pick = turn * (nums[start_idx]) + rec_helper(start_idx + 1, end_idx, -turn)
        last_pick = turn * (nums[end_idx]) + rec_helper(start_idx, end_idx - 1, -turn)

        return turn * max(turn * first_pick, turn * last_pick)

    return rec_helper(0, len(nums) - 1, 1) >= 0
